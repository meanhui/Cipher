...
public class BBGHIBE {
	...
	public BBGHIBEMasterKey Setup(String perperties, int D){
		...
		//generate alpha
		Element alpha = pairing.getZr().newElementFromHash(id, 0, id.length).getImmutable();
		
		// Generate public parameters
		this.g = pairing.getG1().newElementFromHash(id, 0, id.length).getImmutable();
		this.h = pairing.getG1().newElementFromHash(id, 0, id.length).getImmutable();
		this.u = new Element[this.MAX_DEPTH];
		for (int i=0; i<this.u.length; i++){
			this.u[i] = pairing.getG1().newElementFromHash(id, 0, id.length).getImmutable();
		}
	...
	public BBGHIBESecretKey KeyGen(BBGHIBEMasterKey msk, String[] identityVector){
		...
		Element r = pairing.getZr().newElementFromHash(id, 0, id.length).getImmutable();
	 ...
	 * @throws IOException 
	 */
	public Element Encrypt(String[] identityVector) throws IOException{
		//Determine the validity of Identity Vector
		assert(identityVector.length <= this.MAX_DEPTH);
		...
		Element s = pairing.getZr().newElementFromHash(id, 0, id.length).getImmutable();
